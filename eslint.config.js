...
export default [
  ...
  jsdoc.configs['flat/recommended'],
  {
    languageOptions: {
      ...
      globals: {
        ...globals.node,
        // Set by this package's own launch/index.js at boot (never imported) - see
        // global.d.ts's own comment.
        Servable: 'readonly',
      },
    },
    plugins: { jsdoc },
    rules: {
      ...
      // Both real call sites are actual process-lifecycle boundaries (a graceful-shutdown signal
      // handler; a deferred failsafe exit backing up a synchronous throw for fatal boot errors),
      // not error-handling control flow that should throw instead.
  ...
  {
    ignores: ['build/', 'dist/', 'types/', 'node_modules/', '.snapshots/', '**/*.min.js', 'src/_archive/**'],
  },
  prettierConfig,
]
